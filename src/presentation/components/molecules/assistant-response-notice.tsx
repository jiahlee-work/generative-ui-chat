...
export function AssistantResponseNotice(props: AssistantResponseNoticeProps) {
  ...
  const retryButtonClassName = isError
  ...
  return (
    <div className={containerClassName} role={isError ? "alert" : "status"}>
      ...
      {onRetry && (
        <button className={retryButtonClassName} onClick={onRetry} type="button">
          <RotateCcw size={14} />
          재시도
        </button>
      )}
